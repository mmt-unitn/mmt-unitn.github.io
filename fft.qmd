...
4. Con $f0 = 4$; aumentate $K$ fino ad ottenere un’onda quadra perfettamente ricomposta. Quanto vale $K$? C’è qualche nesso con il teorema di Nyquist?
...
$$
\begin{aligned}
\sin(\omega_t) \cdot  \sin(\omega_nt) &= \frac{1}{T}\int_0^T \sin^2(\omega_nt)~dt = \frac{1}{T}\int_0^T\frac{1-2\cos(2\omega_nt)}{2}=\frac{1}{2} \\
\cos(\omega_t) \cdot  \cos(\omega_nt) &= \frac{1}{T}\int_0^T \cos^2(\omega_nt)~dt = \frac{1}{T}\int_0^T\frac{1+2\cos(2\omega_nt)}{2}=\frac{1}{2} \\
\cos(\omega_t) \cdot  \sin(\omega_nt) &= \frac{1}{T}\int_0^T \sin(\omega_nt)\cos(\omega_nt)~dt = \frac{1}{T}\int_0^T\frac{\sin(2\omega_nt)}{2}=0 \\
\cos(\omega_t) \cdot  \cos(\omega_kt) &= \frac{1}{T}\int_0^T \frac{\cos(\omega_nt+\omega_kt)\cos(\omega_nt-\omega_kt)}{2}~dt = 0,\quad n\neq k \\
\sin(\omega_nt) \cdot \sin(\omega_kt) &= \dots = 0 \\
\sin(\omega_nt) \cdot \cos(\omega_kt) &= \dots = 0
\end{aligned}
$$ {#eq-sincos}
...
Passiamo ora a come rappresentare le funzioni in maniera analoga a quanto facciamo con i vettori. La @eq-ft suggerisce infatti la rappresentazione di @fig-2_2, analoga a quella riportata in @fig-2_1.

Secondo quanto rappresentato in @fig-2_2 si potrebbe ragionare indifferentemente (grazie alla relazione di eguaglianza in @eq-ft sulla funzione del tempo oppure su due funzioni (discrete) che rappresentano le componenti della scomposizione secondo Fourier. 

Esiste però una rappresentazione più significativa sia per l’analisi segnali che per la soluzione di equazioni differenziali associate a sistemi lineari. Per determinare tale rappresentazione è necessario elaborare la @eq-ft impiegando la formula di Eulero, ovvero i fasori:
...
\end{aligned}
$$
le quali, sostituite nella @eq-ft, comportano:
...
$$
G_n = g(t) \cdot e^{i\omega_nt}=\frac{1}{T}\int_0^T g(\tau)\left(e^{i\omega_n\tau}\right)^*~d\tau = \frac{1}{T}\int_0^Tg(\tau)e^{-i\omega_n\tau}~d\tau
$$
...
$$
e^{i\omega_nt}\cdot e^{i\omega_kt} = \frac{1}{T}\int_0^T e^{i\omega_n\tau}\left(e^{i\omega_k\tau}\right)^*~d\tau=\delta_{n,k}
$$
dove $\delta_{n,k}$ è il delta di Kroneker. Nel caso in cui $n = k$ basta seguire i seguenti passaggi:
$$
\begin{aligned}
e^{i\omega_nt}\cdot e^{i\omega_kt} &= \frac{1}{T}\int_0^T e^{i\omega_n\tau}\left(e^{i\omega_k\tau}\right)^*~d\tau \\
...
:::{.callout-warning}
Mancano le pagine da 12 a 33 della dispensa originale 
:::
