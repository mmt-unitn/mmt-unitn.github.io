...
È utile tenere sotto mano il relativo cheat sheet: <https://rstudio.github.io/cheatsheets/purrr.pdf>
...
- una funzione anonima definita con la scorciatoria `~ espressione(.)`
...
```{r}
mtcars %>%  
  split(mtcars$cyl) %>%   # divide un dataframe in una lista di dataframe
  map(~ lm(mpg ~ wt, data = .)) %>% 
  map(summary) %>% 
  map_dbl("r.squared") %>% 
  round(3)
...
Si noti che `modify_if` prende due funzioni come argomenti: la prima è il criterio di selezione, chiamato *predicato*, la seconda è la trasformazione da applicare agli elementi selezionati. Il predicato deve essere una funzione che restutuisce un valore logico (`TRUE` o `FALSE`).
...
Infine, `purrr` offre anche funzioni di riduzione, come `reduce`, `accumulate`, `reduce2`, `accumulate2`. Queste funzioni permettono di ridurre una lista di elementi a un singolo valore, secondo un criterio specificato. Ad esempio, applico la funzione addizione per accumulare tutti i numeri da 1 a 10:
```{r}
1:10 %>% 
  reduce(`+`)
```
...
**Si lascia per esercizio modificare il codeice di conseguenza**
:::
...
Ora facciamo la stessa cosa con `purrr`, usando `reduce()`. In questo caso la funzione anonima, come specificato dall'help in linea, prende due argomenti: il primo, `.x`, è il valore accumulato, il secondo, `.y`, è il valore corrente.
...
Possiam essere più sintetici e scrivere:
...
L'obiettivo di `dplyr` è semplificare le operazioni su data frame e tibble, operando per riga o per colonna e permettendo il ragruppamento e la sommarizzazione dei dati.


## Funzioni principali

Le funzioni principali di `dplyr` sono presentate efficacemente nel [cheat sheet](https://rstudio.github.io/cheatsheets/data-transformation.pdf){target="_blank"}.
...
Raccogliere dati provenientio da diverse *sorgenti* in una tabella in formato *wide* è piuttosto comune e intuitivo, ed ha il vantaggio di **raggruppare visivamente** dati coerenti. Per questo motivo è un modo molto comune di presentare i dati. 
...
Realizzare il grafico con i dati in formato *long* è più semplice:
...
Iin generale, il formato *wide* è più compatto, più adatto alla visualizzazione in tabella e alla presentazione dei dati, ma il formato *long* è più flessibile e permette di usare le funzioni di manipolazione dei dati in modo più efficiente.
